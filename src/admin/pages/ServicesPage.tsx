...
const SortableItem = ({ item, onDelete }: { item: Service; onDelete: (id: number) => void }) => {
  const {
    attributes,
    listeners,
    setNodeRef,
      ...
      <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 flex items-center justify-between hover:border-mosk-orange/50 transition-colors">
        <div className="flex items-center gap-4">
          ...
          <div className="relative w-16 h-16 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-700 flex-shrink-0 border border-gray-200 dark:border-gray-600">
            ...
            {item.icon && (
              <div className="absolute top-1 right-1 p-1 bg-white dark:bg-gray-800 rounded-md shadow-sm text-mosk-orange">
          ...
          <div>
            ...
            {item.shortTitle && <p className="text-sm text-gray-500 dark:text-gray-400">{item.shortTitle}</p>}
            <div className="flex flex-wrap gap-2 mt-1">
              ...
              {item.features.length > 3 && <span className="text-xs text-gray-400">+{item.features.length - 3}</span>}
            </div>
          </div>
        </div>
...
const ServicesPage = () => {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (active.id !== over.id) {
      setItems((items) => {
        ...
        const token = localStorage.getItem('adminToken');
        axios.put('http://localhost:3001/api/services/reorder', { items: updates }, {
          headers: { Authorization: `Bearer ${token}` }
  ...
  const confirmDelete = async () => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      await axios.delete(`http://localhost:3001/api/services/${deleteId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      <div className="bg-gray-50 dark:bg-gray-900/50 p-6 rounded-xl border border-gray-200 dark:border-gray-800">
        ...
        {items.length === 0 && (
          <div className="text-center py-12 text-gray-500">
            Henüz hizmet eklenmemiş. "Yeni Hizmet Ekle" butonu ile başlayın.
          </div>
        )}
      </div>
