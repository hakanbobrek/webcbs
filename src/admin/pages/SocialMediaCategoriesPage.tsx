...
const SortableItem = ({ item, onEdit, onDelete }: { item: SocialMediaCategory; onEdit: (item: SocialMediaCategory) => void; onDelete: (id: number) => void }) => {
  const {
    attributes,
    listeners,
    setNodeRef,
...
const SocialMediaCategoriesPage = () => {
  const [items, setItems] = useState<SocialMediaCategory[]>([]);
  const [loading, setLoading] = useState(true);
  const [isEditing, setIsEditing] = useState(false);
  const [editItem, setEditItem] = useState<SocialMediaCategory | null>(null);
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (active.id !== over.id) {
      setItems((items) => {
        ...
        const token = localStorage.getItem('adminToken');
        axios.put('http://localhost:3001/api/social-media-categories/reorder', { items: updates }, {
          headers: { Authorization: `Bearer ${token}` }
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      await axios.delete(`http://localhost:3001/api/social-media-categories/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleEdit = (item: SocialMediaCategory) => {
    setEditItem(item);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editItem) {
        await axios.put(`http://localhost:3001/api/social-media-categories/${editItem.id}`, formData, config);
        toast.success('Kategori güncellendi!');
      } else {
        await axios.post('http://localhost:3001/api/social-media-categories', formData, config);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Sosyal Medya Kategorileri</h1>
        <button 
          onClick={() => {
            setIsEditing(true);
            setEditItem(null);
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <input 
                    ...
                    placeholder="Örn: Proje Güncellemeleri"
                    required
                  />
                </div>
                <div>
