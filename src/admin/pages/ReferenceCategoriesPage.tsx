...
const SortableItem = ({ item, onEdit, onDelete }: { item: ReferenceCategory; onEdit: (item: ReferenceCategory) => void; onDelete: (id: number) => void }) => {
  const {
    attributes,
    listeners,
    setNodeRef,
...
const ReferenceCategoriesPage = () => {
  const [items, setItems] = useState<ReferenceCategory[]>([]);
  const [loading, setLoading] = useState(true);
  const [isEditing, setIsEditing] = useState(false);
  const [editItem, setEditItem] = useState<ReferenceCategory | null>(null);
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (active.id !== over.id) {
      setItems((items) => {
        ...
        const token = localStorage.getItem('adminToken');
        axios.put('http://localhost:3001/api/reference-categories/reorder', { items: updates }, {
          headers: { Authorization: `Bearer ${token}` }
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      await axios.delete(`http://localhost:3001/api/reference-categories/${id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleEdit = (item: ReferenceCategory) => {
    setEditItem(item);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editItem) {
        await axios.put(`http://localhost:3001/api/reference-categories/${editItem.id}`, formData, config);
        toast.success('Kategori güncellendi!');
      } else {
        await axios.post('http://localhost:3001/api/reference-categories', formData, config);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Referans Kategorileri</h1>
        <button 
          onClick={() => {
            setIsEditing(true);
            setEditItem(null);
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <input 
                    ...
                    placeholder="Örn: Kamu Kurumları"
                    required
                  />
                </div>
                <div>
...
export default ReferenceCategoriesPage;
