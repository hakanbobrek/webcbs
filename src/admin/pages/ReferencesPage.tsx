...
const SortableItem = ({ item, onDelete }: { item: Reference; onDelete: (id: number) => void }) => {
  const {
    attributes,
    listeners,
    setNodeRef,
      ...
      <div className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 flex items-center justify-between hover:border-mosk-orange/50 transition-colors">
        <div className="flex items-center gap-4">
          ...
          <div className="relative w-16 h-16 rounded-lg overflow-hidden bg-gray-100 dark:bg-gray-700 flex-shrink-0 border border-gray-200 dark:border-gray-600">
            {item.logoUrl ? (
              <img src={item.logoUrl} alt={item.title} className="w-full h-full object-contain p-2" />
            ) : (
...
const ReferencesPage = () => {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    if (active.id !== over.id) {
      setItems((items) => {
        ...
        const token = localStorage.getItem('adminToken');
        axios.put('http://localhost:3001/api/references/reorder', { items: updates }, {
          headers: { Authorization: `Bearer ${token}` }
  ...
  const confirmDelete = async () => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      await axios.delete(`http://localhost:3001/api/references/${deleteId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Referanslar</h1>
        <Link 
      ...
      <div className="bg-gray-50 dark:bg-gray-900/50 p-6 rounded-xl border border-gray-200 dark:border-gray-800">
        ...
        {items.length === 0 && (
          <div className="text-center py-12 text-gray-500">
            Henüz referans eklenmemiş. "Yeni Referans Ekle" butonu ile başlayın.
          </div>
        )}
      </div>
      ...
      <ConfirmModal
        ...
        title="Referansı Sil"
        message="Bu referansı silmek istediğinizden emin misiniz? Bu işlem geri alınamaz."
      />
